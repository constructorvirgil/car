...
//返回：无
//==============================================================
//void Draw_BMP(byte x0,byte y0,byte x1,byte y1,uint8 bmp[])//此函数显示有问题，应该是byte和uint8的区别
//{ 	
//  word ii=0;
//  byte x,y;
//  
...
void LCD_P6x8Str1(byte x,byte y,uint8 date)//无符号
{  
  byte c=0,i=0,j=0,ch[2]={0};
  ch[0]=date+'0';
    while ((1-j)!=0)
  {
    c =ch[j]-32;
...
void LCD_P6x8Str_1(byte x,byte y,int8 date)//带符号数
{  
  byte c=0,i=0,j=0,ch[2]={0};
  if(date<0)
  {
